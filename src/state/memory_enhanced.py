...
class EnhancedMemory:
    ...
    def __init__(self, agent_id: str, ttl: int = 3600):
        ...
        logger.info("Enhanced memory manager initialized", agent_id=agent_id, ttl=ttl)
    
    async def initialize(self) -> None:
        """初始化内存管理器"""
        try:
            logger.info("Initializing enhanced memory manager", agent_id=self.agent_id)
            ...
            logger.info("Enhanced memory manager initialized successfully", agent_id=self.agent_id)
            
        except Exception as e:
            logger.error("Failed to initialize enhanced memory manager", error=str(e))
            raise
    ...
    async def _schedule_cleanup(self, entry_id: str, ttl: int):
        """调度清理任务"""
    ...
    async def clear(self):
        ...
        self._update_stats()
        logger.info("Enhanced memory cleared", agent_id=self.agent_id) 